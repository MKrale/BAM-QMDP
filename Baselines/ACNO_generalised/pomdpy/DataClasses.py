...
class PositionData(HistoricalData):
    ...
    def create_child(self, action, observation):
        ...
        next_position, is_legal = self.model.make_next_position(
            self.position, action.bin_number
        )
...
class BoxState(DiscreteState):
    ...
    def copy(self):
        return BoxState(self.position, is_terminal=self.terminal, r=self.final_rew)
