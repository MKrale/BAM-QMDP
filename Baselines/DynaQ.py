...
class QBasic:
    ...
    def update_Q(self, s, action, reward, obs):
        ...
        self.Q[s, action] = (1 - self.lr) * self.Q[s, action] + self.lr * (
            reward + self.df * Psi
        )

    def update_T(self, s, action, obs):
        self.T_counter[s, action, obs] += 1
        self.T[s, action] = self.T_counter[s, action] / np.sum(
            self.T_counter[s, action]
        )
    ...
    def run(self, episodes, logging=False):
        ...
        for i in range(episodes):
            ...
            if logging and i % 100 == 0:
                print(
                    "{} / {} runs complete (current avg reward = {}, nmbr steps = {})".format(
                        i,
                        episodes,
                        np.average(rewards[(i - 100) : i]),
                        np.average(steps[(i - 100) : i]),
                    )
                )
...
class QOptimistic(QBasic):
    ...
    def update_Q(self, s, action, reward, obs):
        Psi = 0
        Psi += np.sum(self.T[s, action] * np.max(self.Q, axis=1))
        self.Q_unbiased[s, action] = (1 - self.lr) * self.Q[s, action] + self.lr * (
            reward + self.df * Psi
        )

        self.Q[s, action] = self.Q_unbiased[
            s, action
        ]  # + self.optBias*np.sqrt(self.N_since_last_tried)
        self.N_since_last_tried += 1
        self.N_since_last_tried[s, action] = 1
        self.Q = self.Q + self.optBias * (
            np.sqrt(self.N_since_last_tried - 1) + np.sqrt(self.N_since_last_tried)
        )
