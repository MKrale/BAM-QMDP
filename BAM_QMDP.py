...
class BAM_QMDP:
    ...
    def __init__(self, env:AM_ENV, epsilon = 0.0, nmbr_particles = 100,  offline_training_steps = 0):
...
Rewards Table: {}           
    ...
    def weighted_visits(self, b, action):
        ...
        for s in b:
            if b[s] > 1/self.StateSize:
                ...
                p_total += b[s]
                w += min(1, (np.sum(self.alpha[s,a,:]) / self.NmbrOptimiticTries)) * b[s]
        # print(w)
        if p_total == 0:
            return 1
        return w / p_total
    ...
    def check_validity_belief(self, b:dict):
        if self.doneState in b:
            ...
            if p_done == 1:
                return dict(zip(range(self.StateSize), np.repeat(1/self.StateSize, self.StateSize)))
    ...
    def update_Q_lastStep_only(self,S1, action, reward=0, isDone = False, isReal = True):
        ...
        for s1 in S1:
            ...
            if self.optimism_type == "RMAX+":
                self.QTable[s1,action] = totQ + max(0, max(0, 1-totQ) * (self.NmbrOptimiticTries - thisAlpha) / self.NmbrOptimiticTries)
