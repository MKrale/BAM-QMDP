...
class MDP(object):
    ...
    def __init__(
        ...
        """
        assert p_diabetes >= 0 and p_diabetes <= 1, "Invalid p_diabetes: {}".format(
            p_diabetes
        )
        ...
        if policy_array is not None:
            ...
            if policy_idx_type == "obs":
                ...
                assert (
                    policy_array.shape[0] == State.NUM_HID_STATES * State.NUM_OBS_STATES
                )
    ...
    def get_new_state(self, state_idx=None, idx_type="obs", diabetic_idx=None):
        ...
        * This option will set glucose to a normal level
        ...
        if option in ["random", "random_cond_diab"]:
            ...
            init_state = State(
                state_idx=state_idx, idx_type=idx_type, diabetic_idx=diabetic_idx
            )
    ...
    def generate_random_state(self, diabetic_idx=None):
        ...
        hr_state = np.random.choice(np.arange(3), p=np.array([0.25, 0.5, 0.25]))
        sysbp_state = np.random.choice(np.arange(3), p=np.array([0.25, 0.5, 0.25]))
        # percoxyg w.p. [.2, .8]
        percoxyg_state = np.random.choice(np.arange(2), p=np.array([0.2, 0.8]))

        if diabetic_idx == 0:
            glucose_state = np.random.choice(
                np.arange(5), p=np.array([0.05, 0.15, 0.6, 0.15, 0.05])
            )
        else:
            glucose_state = np.random.choice(
                np.arange(5), p=np.array([0.01, 0.05, 0.15, 0.6, 0.19])
            )
        antibiotic_state = 0
        vaso_state = 0
        vent_state = 0
    ...
    def transition_fluctuate(
        self, hr_fluctuate, sysbp_fluctuate, percoxyg_fluctuate, glucose_fluctuate
    ):
        """
    ...
    def transition(self, action):
        ...
        self.transition_fluctuate(
            hr_fluctuate, sysbp_fluctuate, percoxyg_fluctuate, glucose_fluctuate
        )
